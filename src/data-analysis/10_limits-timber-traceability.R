...
# the study "Mapping the limits to timber traceability at origin: The case of
# Pará, Brazil".
...
# directly if you come across any issues or suggestions for improvement.
...
mun_pa <- mun |>
  ...
  st_transform(
    crs = "+proj=aea +lat_0=-12 +lon_0=-54 +lat_1=-2 +lat_2=-22 
               +x_0=5000000 +y_0=10000000 +ellps=GRS80 +units=m +no_defs"
  )
...
vol_by_lp <- vol_by_lp |>
  mutate(
    LP_REF = str_remove(LP_REF, "AUTEX "),
...
lp_coord <- join_p_lp_data |>
  select(-GEOM) |>
  drop_na(c(X, Y)) |>
  st_as_sf(coords = c("X", "Y"), crs = "EPSG:4674") |>
  st_transform(
    crs = "+proj=aea +lat_0=-12 +lon_0=-54 +lat_1=-2 +lat_2=-22 
               +x_0=5000000 +y_0=10000000 +ellps=GRS80 +units=m +no_defs"
  )
...
data_allspp_df1 <- join_p_lp_data |>
  mutate(
    LP_SOURCE = case_when(
      is.na(LP_SOURCE) ~ "UNDETERMINED",
      .default = LP_SOURCE
    )
  ) |>
  mutate(
    LP_STATUS = case_when(
      is.na(LP_STATUS) ~ "UNDETERMINED",
      .default = LP_STATUS
    )
  )
# Check LP_SOURCE
  ...
  mutate(
    STATUS_GROUP1 = case_when(
      ...
      LP_STATUS %in%
        ...
        "INVALID",
      LP_STATUS %in% c("EMITIDA OFERTA", "ESTORNADO ITEM") ~ "AUTEX",
      .default = LP_STATUS
    )
  )
...
data_allspp_df4 <- data_allspp_df3 |>
  mutate(
    STATUS_GROUP3 = case_when(
      LP_STATUS %in% c("EXPIRED") ~ LP_STATUS,
...
data_allspp_df5 <- data_allspp_df4 |>
  mutate(
    STATUS_GROUP1 = fct_relevel(STATUS_GROUP1, c("VALID", "INVALID", "AUTEX"))
  ) |>
  mutate(
    STATUS_GROUP2 = fct_relevel(
      STATUS_GROUP2,
      c(
        "VALID",
        "MISSING ACTIVATION DATE",
...
stat_obj1 <- lp_pol |>
  ...
  summarise(SUM_VOL = sum(SUM_VOL), n = n(), SUM_AREA = sum(AREA_HA)) |>
  mutate(
    perc_tot_vol = SUM_VOL / tot_vol_rw * 100,
...
lp_coord_no_pol <- lp_coord |>
  filter(!LP_REF %in% lp_pol_key, !grepl(c("AUTEX"), LP_SOURCE))
...
lp_coord_outside_upa_overlap <- lp_coord_no_pol |>
  filter(!LP_REF %in% overlap_coord_pol_key)
...
overlap_coord_atlas_key <- overlap_coord_atlas |>
  st_drop_geometry() |>
  distinct(LP_REF) |>
  pull()

# Add the key from the previous removals so that we can get the remaining
overlap_coord_upa_amf_atlas_key <- c(
  overlap_coord_pol_key,
  overlap_coord_amf_key,
  overlap_coord_atlas_key
)
lp_coord_no_pol |>
  st_drop_geometry() |>
...
lp_coord_residual_atlas <- lp_coord_no_pol |>
  filter(!LP_REF %in% overlap_coord_upa_amf_atlas_key)
...
stat_obj6 <- lp_coord_residual_atlas |>
  ...
  distinct(LP_REF, SUM_VOL, .keep_all = TRUE) |>
  summarise(SUM_VOL = sum(SUM_VOL), n = n(), SUM_AREA = sum(AREA_HA)) |>
  mutate(perc_tot_vol = SUM_VOL / tot_vol_rw * 100, class = "Undetermined") |>
...
rm(
  ...
  overlap_coord_amf,
  overlap_coord_cnfp_atlas,
  overlap_coord_upa_amf_key,
  overlap_coord_upa_amf_atlas_key,
  lp_coord_residual_atlas,
  lp_coord_no_pol_cnt,
  lp_coord_outside_amf_overlap,
  lp_coord_outside_upa_overlap
)
...
bind_rows(stat_obj1, stat_obj2, stat_obj3, stat_obj4, stat_obj5, stat_obj6) |>
  filter(class != "-") |>
  filter(
    grepl("Coordinates|Undetermined", class),
    class != "AUTEX-UMF Coordinates"
  ) |>
  adorn_totals()
...
simex_dissolved_area <- simex_dissolved |>
  st_area() |>
  sum() |>
  units::drop_units()
...
simex_outside_upa_area <- simex_outside_upa |>
  st_area() |>
  sum() |>
  units::drop_units()
...
simex_clusters_df1 <- simex_clusters |>
  mutate(
    AREA_SIMEX_HA = units::set_units(st_area(simex_clusters), "hectares")
  ) |>
  drop_units()

# Subset the Simex clusters unionized with the intersections of polygons available
# from AUTEF-UPA
simex_clusters_autef_upa_pol <- st_intersection(
  simex_clusters_df1,
  lp_pol_simex
)
...
simex_clusters_autef_upa_pol_df1 <- simex_clusters_autef_upa_pol |>
  mutate(
    INT_AREA_SIMEX_HA = units::set_units(
      st_area(simex_clusters_autef_upa_pol),
      "hectares"
    )
  ) |>
  drop_units() |>
...
amf_umf_simex_dissolved <- amf_umf_simex |>
  st_collection_extract() |>
  st_union()
...
# Overlap removal
simex_outside_amf_umf <- st_difference(
  simex_outside_upa_dissolved,
  amf_umf_dissolved
)
simex_outside_amf_umf_dissolved <- st_union(simex_outside_amf_umf)
simex_outside_amf_umf_area <- simex_outside_amf_umf |>
  st_area() |>
  sum() |>
  units::drop_units()
...
simex_clusters_amf_umf <- st_intersection(
  simex_clusters_df1,
  amf_umf_simex_dissolved
)
...
simex_clusters_amf_umf_df1 <- simex_clusters_amf_umf |>
  mutate(
    INT_AREA_SIMEX_HA = units::set_units(
      st_area(simex_clusters_amf_umf),
      "hectares"
    )
  ) |>
  drop_units() |>
...
simex_outside_amf_umf_dissolved_df1 <- simex_outside_amf_umf_dissolved |>
  ...
  st_collection_extract() |>
  st_cast("POLYGON")
simex_outside_amf_umf_dissolved_df2 <- st_union(
  simex_outside_amf_umf_dissolved_df1
)

atlas_pl_aru_simex <- st_intersection(
  simex_outside_amf_umf_dissolved_df2,
  atlas_pl_aru_valid_dissolved
)
atlas_pl_aru_simex_area <- atlas_pl_aru_simex |>
  st_area() |>
  sum() |>
  units::drop_units()
...
simex_outside_atlas <- st_difference(
  simex_outside_amf_umf_dissolved_df2,
  atlas_pl_aru_valid_dissolved
)
simex_outside_atlas_area <- simex_outside_atlas |>
  st_area() |>
  sum() |>
  units::drop_units()
...
atlas_sigef_simex <- st_intersection(
  simex_outside_amf_umf_dissolved_df2,
  atlas_sigef_dissolved
)
atlas_sigef_simex_area <- atlas_sigef_simex |>
  st_area() |>
  sum() |>
  units::drop_units()
...
simex_outside_sigef <- st_difference(
  simex_outside_amf_umf_dissolved_df2,
  atlas_sigef_dissolved
)
simex_outside_sigef_area <- simex_outside_sigef |>
  st_area() |>
  sum() |>
  units::drop_units()
...
atlas_tl_dissolved_area <- atlas_tl_dissolved |>
  st_area() |>
  sum() |>
  units::drop_units()
...
atlas_tl_simex <- st_intersection(
  simex_outside_sigef_dissolved_df2,
  atlas_tl_dissolved
)
atlas_tl_simex_area <- atlas_tl_simex |>
  st_area() |>
  sum() |>
  units::drop_units()
...
# Create Simex object without SIGEF overlap/Simex areas outside UPA-AMF-UMF-SIGEF
simex_outside_tl <- st_difference(
  simex_outside_sigef_dissolved_df2,
  atlas_tl_dissolved
)
...
atlas_ql_dissolved_area <- atlas_ql_dissolved |>
  st_area() |>
  sum() |>
  units::drop_units()
...
atlas_ql_simex <- st_intersection(
  simex_outside_tl_dissolved_df2,
  atlas_ql_dissolved
)
atlas_ql_simex_area <- atlas_ql_simex |>
  st_area() |>
  sum() |>
  units::drop_units()
...
# Create Simex object without SIGEF overlap/Simex areas outside UPA-AMF-UMF-SIGEF
simex_outside_ql <- st_difference(
  simex_outside_tl_dissolved_df2,
  atlas_ql_dissolved
)
...
atlas_aru_dissolved_area <- atlas_aru_dissolved |>
  st_area() |>
  sum() |>
  units::drop_units()
...
atlas_aru_simex <- st_intersection(
  simex_outside_ql_dissolved_df2,
  atlas_aru_dissolved
)
atlas_aru_simex_area <- atlas_aru_simex |>
  st_area() |>
  sum() |>
  units::drop_units()
...
# Create Simex object without SIGEF overlap/Simex areas outside UPA-AMF-UMF-SIGEF
simex_outside_aru <- st_difference(
  simex_outside_ql_dissolved_df2,
  atlas_aru_dissolved
)
...
atlas_car_dissolved_area <- atlas_car_dissolved |>
  st_area() |>
  sum() |>
  units::drop_units()
...
atlas_car_simex <- st_intersection(
  simex_outside_aru_dissolved_df2,
  atlas_car_dissolved
)
atlas_car_simex_area <- atlas_car_simex |>
  st_area() |>
  sum() |>
  units::drop_units()
...
# Create Simex object without SIGEF overlap/Simex areas outside UPA-AMF-UMF-SIGEF
simex_outside_car <- st_difference(
  simex_outside_aru_dissolved_df2,
  atlas_car_dissolved
)
...
rm(
  ...
  atlas_ql_area,
  atlas_ql_dissolved,
  atlas_ql_dissolved_area,
  atlas_ql_simex,
  atlas_ql_simex_area,
  atlas_aru,
  atlas_aru_area,
  atlas_aru_dissolved,
  atlas_aru_dissolved_area,
  atlas_aru_simex,
  atlas_aru_simex_area,
  atlas_car,
  atlas_car_area,
  atlas_car_dissolved,
  atlas_car_dissolved_area,
  atlas_car_simex,
  atlas_car_simex_area,
  atlas_pl_aru_simex_area,
  atlas_sigef_simex_area,
  lp_pol_area,
  lp_pol_area_dissolved
)

rm(
  simex_outside_sigef_dissolved_df1,
  simex_outside_sigef_dissolved_df2,
  simex_outside_tl_dissolved_df1,
  simex_outside_tl_dissolved_df2,
  simex_outside_ql_dissolved_df1,
  simex_outside_ql_dissolved_df2,
  simex_outside_aru_dissolved_df1,
  simex_outside_aru_dissolved_df2,
  simex_outside_car_dissolved_df1,
  simex_outside_car_dissolved_df2,
  simex_outside_sigef,
  simex_outside_tl,
  simex_outside_ql,
  simex_outside_aru,
  simex_outside_car,
  simex_outside_sigef_area,
  simex_outside_tl_area,
  simex_outside_ql_area,
  simex_outside_aru_area,
  simex_outside_car_area,
...
overlap_coord_atlas_simex <- st_intersection(
  simex_outside_amf_umf_dissolved_df2,
  overlap_coord_atlas_dissolved
)
overlap_coord_atlas_simex_area <- overlap_coord_atlas_simex |>
  st_area() |>
  sum() |>
  units::drop_units()
...
# Subset the Simex clusters unionized with the intersections of polygons available
# from AUTEF-UPA AMF/ AUTEX-UMF
simex_clusters_pl_aru <- st_intersection(
  simex_clusters_df1,
  overlap_coord_atlas_simex
)
...
# SIGEF Object
atlas_sigef_coord <- overlap_coord_atlas |>
  filter(grepl(c("SIGEF"), NM_SUBCLASS))
atlas_sigef_coord_area <- atlas_sigef_coord |>
  st_area() |>
  sum() |>
  units::drop_units()

atlas_sigef_coord_dissolved <- st_union(atlas_sigef_coord)
atlas_sigef_coord_dissolved_area <- atlas_sigef_coord_dissolved |>
  st_area() |>
  sum() |>
  units::drop_units()
...
# Intersection of SIMEX and SIGEF pols
atlas_sigef_coord_simex <- st_intersection(
  simex_outside_amf_umf_dissolved_df2,
  atlas_sigef_coord_dissolved
)
atlas_sigef_coord_simex_area <- atlas_sigef_coord_simex |>
  st_area() |>
  sum() |>
  units::drop_units()
...
# Create Simex object without SIGEF overlap/Simex areas outside UPA-AMF-UMF-SIGEF
simex_outside_sigef_coord <- st_difference(
  simex_outside_amf_umf_dissolved_df2,
  atlas_sigef_coord_dissolved
)
simex_outside_sigef_coord_area <- simex_outside_sigef_coord |>
  st_area() |>
  sum() |>
  units::drop_units()
...
simex_outside_sigef_coord_dissolved_df1 <- simex_outside_sigef_coord |>
  ...
  st_collection_extract() |>
  st_cast("POLYGON")
simex_outside_sigef_coord_dissolved_df2 <- st_union(
  simex_outside_sigef_coord_dissolved_df1
)

# Intersection of SIMEX and Terra Logal pols
atlas_tl_simex_coord <- st_intersection(
  simex_outside_sigef_coord_dissolved_df2,
  atlas_tl_dissolved
)
atlas_tl_simex_coord_area <- atlas_tl_simex_coord |>
  st_area() |>
  sum() |>
  units::drop_units()
...
# Create Simex object without SIGEF overlap/Simex areas outside UPA-AMF-UMF-SIGEF
simex_outside_tl_coord <- st_difference(
  simex_outside_sigef_coord_dissolved_df2,
  atlas_tl_dissolved
)
...
simex_outside_tl_coord_dissolved_df1 <- simex_outside_tl_coord |>
  ...
  st_collection_extract() |>
  st_cast("POLYGON")
simex_outside_tl_coord_dissolved_df2 <- st_union(
  simex_outside_tl_coord_dissolved_df1
)

# Intersection of SIMEX and Quilombola territories pols
atlas_ql_simex <- st_intersection(
  simex_outside_tl_coord_dissolved_df2,
  atlas_ql_dissolved
)
atlas_ql_simex_area <- atlas_ql_simex |>
  st_area() |>
...
# Create Simex object without SIGEF overlap/Simex areas outside UPA-AMF-UMF-SIGEF
simex_outside_ql_coord <- st_difference(
  simex_outside_tl_coord_dissolved_df2,
  atlas_ql_dissolved
)
...
simex_outside_ql_coord_dissolved_df1 <- simex_outside_ql_coord |>
  ...
  st_collection_extract() |>
  st_cast("POLYGON")
simex_outside_ql_coord_dissolved_df2 <- st_union(
  simex_outside_ql_coord_dissolved_df1
)

# Intersection of SIMEX and ARU pols
atlas_aru_simex <- st_intersection(
  simex_outside_ql_coord_dissolved_df2,
  atlas_aru_dissolved
)
atlas_aru_simex_area <- atlas_aru_simex |>
  st_area() |>
...
# Create Simex object without SIGEF overlap/Simex areas outside UPA-AMF-UMF-SIGEF
simex_outside_aru_coord <- st_difference(
  simex_outside_ql_coord_dissolved_df2,
  atlas_aru_dissolved
)
...
simex_outside_aru_coord_dissolved_df1 <- simex_outside_aru_coord |>
  ...
  st_collection_extract() |>
  st_cast("POLYGON")
simex_outside_aru_coord_dissolved_df2 <- st_union(
  simex_outside_aru_coord_dissolved_df1
)

# Intersection of SIMEX and CAR pols
atlas_car_simex <- st_intersection(
  simex_outside_aru_coord_dissolved_df2,
  atlas_car_dissolved
)
atlas_car_simex_area <- atlas_car_simex |>
  st_area() |>
...
# Create Simex object without SIGEF overlap/Simex areas outside UPA-AMF-UMF-SIGEF
simex_outside_car_coord <- st_difference(
  simex_outside_aru_coord_dissolved_df2,
  atlas_car_dissolved
)
simex_outside_car_coord_area <- simex_outside_car_coord |>
  st_area() |>
  sum() |>
  units::drop_units()
...
# Create centroid with the listed area of Simex
und <- bind_rows(
  simex_clusters_autef_upa_pol_df1,
  simex_clusters_amf_umf_df1,
  simex_clusters_pl_aru_df1
)

# Subset the Simex clusters for those polygons outside/UNDETERMINED
simex_clusters_und <- st_difference(
  st_union(simex_clusters_df1),
  st_union(und)
) |>
  st_collection_extract() |>
  st_cast("POLYGON") |>
...
non_simex_umf_diss <- st_difference(simex_dissolved, umf_cnfp_dissolved) |>
  st_make_valid()
...
simex_outside_cnpf <- st_difference(
  simex_outside_atlas_dissolved,
  cnpf_nd_ti_ucpi_ucus
)
...
rm(
  ...
  amf_umf_dissolved,
  amf_umf_simex,
  atlas_pl_aru_dissolve,
  atlas_pl_aru_simex,
  simex_outside_amf_umf,
  simex_outside_amf_umf_area,
  umf_dissolved,
  umf_simex,
  umf_simex_area
)
...
share_upa_exploited_df3 <- share_upa_exploited_df2 |>
  mutate(
    AREA_HA_EXP = units::set_units(st_area(share_upa_exploited_df2), "hectares")
  ) |>
...
share_upa_exploited_df4 <- share_upa_exploited_df1 |>
  mutate(
    AREA_HA_EXP = units::set_units(st_area(share_upa_exploited_df1), "hectares")
  ) |>
  select(LP_REF, AREA_HA_EXP)

# Comparing overlaps
area_pre_union <- share_upa_exploited_df4 |>
  st_drop_geometry() |>
  adorn_totals() |>
  slice_max(AREA_HA_EXP) |>
  pull(AREA_HA_EXP)
area_post_union <- share_upa_exploited_df3 |>
  st_drop_geometry() |>
  adorn_totals() |>
  slice_max(AREA_HA_EXP) |>
  pull(AREA_HA_EXP)
...
colors <- c(
  "AUTEF polygons, more than 6% of area exploited" = "#2D609B",
...
figS4B <- ggplot() +
  ...
  geom_text(
    aes(x = 29, y = 0.8, label = "Legal limit"),
    angle = 90,
    color = "black",
    size = 4
  ) +
  geom_step(
    data = harv_int_not6perc,
    aes(
      x = HARVEST_INTENSITY,
      y = csv,
      color = "AUTEF polygons, more than 6% of area exploited"
    ),
    size = 0.6
  ) +
  geom_step(
    ...
    aes(
      x = HARVEST_INTENSITY,
      y = csv,
      color = "AUTEF polygons, up to 6% of area exploited"
    ),
    size = 0.6
  ) +
  labs(
    x = expression(paste("Harvest Intensity (", m^3, ha^-1, ")", sep = "")),
...
figS4A <- ggplot() +
  geom_step(
    data = data_figS4,
    aes(x = SHARE_SIMEX, y = csv, color = "AUTEF-UPA polygons, area exploited"),
    size = 0.6
  ) +
  labs(
    x = expression(paste("Area Simex/Area AUTEF-UPA")),
    y = "Cumulative Share of Volume (%)",
    color = NULL,
    tag = "A"
  ) +
...
# Clean Environemnet
rm(
  share_upa_exploited_df1,
  share_upa_exploited_df2,
  share_upa_exploited_df3,
...
coord_simex_precise_int_all_dissolved_area <- coord_simex_precise_int_all_dissolved |>
  st_area() |>
  sum()
...
fig1a_base <- ggplot() +
  ...
  scale_fill_gradientn(
    name = "Roundwood production\n by municipality",
    colours = c("#E4D7BD", "#CFB889", "#B99956", "#A37824"),
    na.value = "#EAEDE9",
...
data_fig1_df3 <- data_fig1_df2 |>
  ...
  mutate(
    TIER2 = fct_relevel(TIER2, c("Polygons", "Coordinates", "Undetermined"))
  )
...
data_fig1 <- data_fig1_df3 |>
  ...
  mutate(X = map_dbl(geometry, 1), Y = map_dbl(geometry, 2))


# Mapping volume entering the supply chain by Valid, Invalid, AUTEX
fig1a <- fig1a_base +
  geom_point(
    data = filter(data_fig1, STATUS_GROUP1 != "UNDETERMINED"),
  ...
  scale_size(
    name = "Roundwood production \n by Logging Permit",
    limits = c(min(data_fig1$SUM_VOL), max(data_fig1$SUM_VOL)),
    range = c(0, 10),
    #labels = label_comma(suffix = " m\u00B3"))+
    labels = label_number(scale = 1 / 10000, suffix = " Mm\u00B3         ")
  ) +
  scale_colour_manual(
    name = "Logging Permits \n geolocated by...",
    values = c("#2D609B", "#9C5651", "grey40"),
    labels = c("Polygons", "Coordinates", "Undetermined                     ")
  ) +
...
ggsave(filename = "./results/figS2.pdf", width = 17, height = 6, bg = "white")
...
area_by_mun_df2 <- area_by_mun_df1 |>
  mutate(
    AREA_SIMEX_HA = units::set_units(st_area(area_by_mun_df1), "hectares")
  ) |>
  select(-c(code_state, abbrev_state, code_region, name_state, name_region)) |>
  drop_units()
...
centroid_simex_undetermined <- centroid_simex_undetermined |>
  rename(
    x = "st_cast.st_collection_extract.st_difference.st_union.simex_clusters_df1..."
  )
...
data_fig1bv1 <- st_centroid(centroid_simex_clustersv1) |>
  ...
  mutate(X = map_dbl(x, 1), Y = map_dbl(x, 2))
...
fig1bv1 <- fig1b_base +
  ...
  scale_size(
    name = "Area of forest exploited \n by Logging Permit",
    limits = c(
      min(data_fig1bv1$INT_AREA_SIMEX_HA),
...
fig1bv2 <- fig1b_base +
  ...
  scale_size(
    name = "Area of forest exploited\nby logging Permit",
    limits = c(
      min(data_fig1bv2$INT_AREA_SIMEX_HA),
...
ggsave(filename = "./results/figS3.pdf", width = 17, height = 6, bg = "white")
...
fig1d <- fig1d1 +
  geom_rect(
    aes(xmin = 0.66, xmax = 1.34, ymin = 15, ymax = 55),
    color = "black",
...
missing_extraction_status <- st_filter(
  data_status_missing_extraction,
  simex_dissolved,
  .predicate = st_disjoint
)
...
colors <- c(
  "AUTEF polygons, area exploited" = "#2D609B",
...
fig2 <- ggplot() +
  ...
  geom_step(
    data = harv_int_lp_exploited,
    aes(
      x = HARVEST_INTENSITY,
      y = csv,
      color = "AUTEF polygons, area exploited"
    ),
    size = 0.6
  ) +
  geom_step(
    data = harv_int_lp_notexploited,
    aes(
      x = HARVEST_INTENSITY,
      y = csv,
      color = "AUTEF polygons, area not exploited"
    ),
    size = 0.6
  ) +
  geom_step(
    data = harv_int_lp_coordinates,
    aes(x = HARVEST_INTENSITY, y = csv, color = "AUTEF coordinates"),
    size = 0.6
  ) +
  geom_step(
    data = harv_int_lp_autex,
    aes(x = HARVEST_INTENSITY, y = csv, color = "AUTEX"),
    size = 0.6
  ) +
  labs(
    x = expression(paste("Harvest Intensity (", m^3, ha^-1, ")", sep = "")),
...
# Saving fig2
ggsave(filename = "./results/fig2.pdf", width = 8, height = 7, bg = "white")
