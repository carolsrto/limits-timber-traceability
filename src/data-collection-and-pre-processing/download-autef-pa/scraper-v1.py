...
class Scraper():
    ...
    def get_gfprod(self, id):
        dfts = []
        querystring = {"page":"1","id":f"{id}"}
    ...
    def get_ids(self, url, prods):
        try:
            r = requests.request("GET", url + "?page=1",  headers={"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/111.0.0.0 Safari/537.36"})
            ...
            with Pool(P_COUNT) as p:
                res = p.starmap(self.get_id, zip(i, repeat(url), repeat(prods)), chunksize=int(len(i)/4))
                for r in res:
                    pass
        except: print(traceback.format_exc())
    ...
    def get_xlsx(self, url):
        ...
        with Pool(P_COUNT) as p:
                ...
                res = p.starmap(self.get_xlsx_info, zip(i, repeat(url)), chunksize=int(len(i)/4))
                for r in res:
                    pass
...
def main():
    ...
    while choice != 0:
        ...
        if choice == "autef":
            ...
            s.get_ids("http://portaldatransparencia.semas.pa.gov.br/portal-da-transparencia-api/api/v1/autef/", prod)
            if not s.df.empty:
            ...
            print("COLETANDO PMF")
            prod = False
            s.get_ids("http://portaldatransparencia.semas.pa.gov.br/portal-da-transparencia-api/api/v1/pmf/", prod)
            ...
            if int(choice) == 1:
                ...
                print("COLETANDO GF")
                s.get_ids("http://portaldatransparencia.semas.pa.gov.br/portal-da-transparencia-api/api/v1/guia-florestal/", prod)
                if not s.df.empty:
                ...
                s.get_xlsx("http://portaldatransparencia.semas.pa.gov.br/portal-da-transparencia-api/api/v1/guia-florestal/")
                ...
                s.get_ids("http://portaldatransparencia.semas.pa.gov.br/portal-da-transparencia-api/api/v1/guia-florestal/", "only")
                with open("last_pages.obj", 'wb') as f:
                    s.lp["gf_p"] = 1
                    pickle.dump(s.lp, f)
        else:
